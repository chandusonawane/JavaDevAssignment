...
public class RestConsumerApplication implements CommandLineRunner {
	...
	@Autowired
	public RestConsumerApplication(CountryLookupApplicationProxy countryLookupApplicationProxy,
			SortByCountryNameApp sortByCountryNameApp) {
	...
	public void run(String... args) throws IOException {
		...
		for (String ipAddress : listOfIps) {
			System.out.println(ipAddress + "  " + countryLookupApplicationProxy.getCountryCodeByIp(ipAddress) + "   "
					+ countryLookupApplicationProxy.getCountryNameByIp(ipAddress));
		}
	}
}
